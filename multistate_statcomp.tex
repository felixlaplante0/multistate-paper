...
\[
\log \mathcal{L}_\text{marginal}(\hat{\theta}; x) = \mathbb{E}_{b \sim p(\cdot \mid x, \hat{\theta})}\left[ \log \mathcal{L}(\hat{\theta}; x, b) \right] - \mathbb{E}_{b \sim p(\cdot \mid x, \hat{\theta})}\left[ \log p(b \mid x, \hat{\theta}) \right],
\]
and replacing $p(b \mid x, \hat{\theta})$ by a Gaussian distribution with covariance matrix $\Sigma$, we obtain the approximation
\[
\log \mathcal{L}_\text{marginal}(\hat{\theta}; x) \approx \mathbb{E}_{b \mid x, \hat{\theta}}\left[ \log \mathcal{L}(\hat{\theta}; x, b) \right] - \frac{1}{2} \left( \log \det \Sigma + q\left( \log(2\pi) + 1 \right) \right),
\]
