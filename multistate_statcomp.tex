...
\begin{algorithm}[H]
    ...
    \begin{algorithmic}[1]
        ...
        \Require data $x$; initial parameters $\theta^{(0)}$; step sizes $(\eta_t)_{t \geq 0}$ with $\sum_t \eta_t = +\infty$, $\sum_t \eta_t^2 < +\infty$; batch size $K$; sampler $p^{\mathrm{MCMC}}$; stopping criterion.
        \Statex
        \State $t \gets 0$
        \While{not converged}
        \State Draw $b_k \sim p(\cdot \mid x, \theta^{(t)}) \approx p^\mathrm{MCMC}$.
...
In practice, as previously noted, the marginal $\log$-likelihood
\[
