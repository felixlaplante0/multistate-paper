...
\]
and replacing $p(b \mid x, \hat{\theta})$ by a Gaussian distribution with covariance matrix $\Sigma$, we obtain the approximation
\[
\log \mathcal{L}_\mathrm{marginal}(\hat{\theta}; x) \approx \mathbb{E}_{b \mid x, \hat{\theta}}\left[ \log \mathcal{L}(\hat{\theta}; x, b) \right] - \frac{1}{2} \left( \log \det \Sigma + q\left( \log(2\pi) + 1 \right) \right),
\]
where $q$ denotes the dimension of the latent vector. This approximation replaces the intractable posterior entropy by the closed-form entropy of a Gaussian distribution.
...
\]
where the samples $\{ \mathcal{T}_i^{(k)} \}_{1 \leq k \leq B}$ are generated using a Markov chain Monte Carlo algorithm targeting the distribution $\mathscr{L}(b_i \mid \mathcal{Y}_i(t), \mathcal{S}_i(t), \hat{\theta})$ in conjunction with Algorithm~\ref{alg:trajectory-sim}.
