...
While both joint models and multi-state models offer powerful tools for analyzing disease progression, they emphasize different aspects of the underlying process. Multi-state models capture the sequence and timing of discrete transitions between states, whereas joint models focus on the evolution of continuous longitudinal markers and their association with event risk. Existing joint multi-state models, such as that of Ferrer et al.~\citep*{ferrer2016joint}, are restricted to linear mixed-effects submodels and to acyclic Markov transition graphs, are limited to linear mixed-effects submodels and acyclic Markov transition graphs, since their reliance on binary censoring indicators inherently prevents modeling recurrent events. However, many biological processes involve cyclic or semi-Markov transitions and nonlinear biomarker dynamics. These cannot be handled by existing frameworks, motivating the present general formulation.
...
\begin{align*}
    ...
    \exp \left[-\sum_{s: (S_{i\ell}, s)\in E}
        \Lambda_i^{s \mid S_{i\ell}}(t \mid T_{i\ell})\right].
\end{align*}

Conditional transition probabilities are
\[
    \mathbb{P}(S_{i,\ell+1} = k' \mid T_{i,\ell+1}, T_{i\ell}, S_{i\ell}) = \frac{\lambda_i^{k' \mid S_{i\ell}}(T_{i,\ell+1} \mid T_{i\ell})}{\sum_{s: (S_{i\ell},s)\in E}\lambda_i^{s \mid S_{i\ell}}(T_{i, \ell+1}\mid T_{i\ell})}.
\]
...
\begin{description}
    ...
    \item[\textnormal{\textbf{Longitudinal likelihood:}}]
        \begin{align*}
           &p(Y_i \mid b_i, X_i, \theta) = \prod_{j = 1}^{n_i} \frac{(2 \pi)^{-d/2}}{\det(R)^{1/2}} \exp\left( -\tfrac{1}{2} \left(Y_{ij} - h(t_{ij}, \psi_i) \right)^T R^{-1} \left( Y_{ij} - h(t_{ij}, \psi_i) \right) \right), \\
    ...
    \item[\textnormal{\textbf{Semi-Markov likelihood:}}]
          \begin{equation}
              \begin{aligned}
                  p(\mathcal{T}_i \mid b_i, X_i, C_i, \theta) = &\prod_{\ell=0}^{m_i-1} p\left((T_{i(\ell+1)}, S_{i(\ell+1)}) \mid b_i, X_i, (T_{i\ell}, S_{i\ell}), \theta\right) \\
     &\exp\left( -\sum_{s: (S_{im_i}, s) \in E} \Lambda_i^{s \mid S_{im_i}} (C_i \mid b_i, X_i, T_{im_i}, \theta) \right).
              \end{aligned}
...
\[
    \mathcal{L}_\mathrm{complete}(\theta  ; X, Y, \mathcal{T}, C, b) = \prod_{i=1}^n p(Y_i \mid b_i, X_i, \theta) \, p(\mathcal{T}_i \mid b_i, X_i, C_i, \theta) \, p(b_i \mid \theta).
\]
...
\[
    \nabla_\theta \log \mathcal{L}_\mathrm{marginal}(\theta; x) = \mathbb{E}_{b \sim p(\cdot \mid x, \theta)}\left[ \nabla_\theta \log \mathcal{L}_{\mathrm{complete}}(\theta; x, b) \right].
\]
...
\begin{algorithm}[H]
    ...
    \begin{algorithmic}[1]
        ...
        \Require data $x$; initial parameters $\theta^{(0)}$; step sizes $(\eta_t)_{t \geq 0}$ with $\sum_t \eta_t = +\infty$, $\sum_t \eta_t^2 < +\infty$; number of parallel chains $K$; MCMC samplers $(p_k^{\mathrm{MCMC}})_{1 \leq k \leq K}$; stopping criterion.
        \Statex
        \State $t \gets 0$
        \While{not converged}
        \State For each $k \in \{ 1, \dots, K \}$, draw $b_k \sim p(\cdot \mid x, \theta^{(t)}) \approx p_k^\mathrm{MCMC}$.
...
\[
    \mathrm{AIC} \coloneqq -2 \log \mathcal{L}_\mathrm{marginal}(\hat{\theta}; X, Y, \mathcal{T}, C) + 2 k,
\]
...
An alternative consists in approximating the posterior distribution $p(b \mid x, \hat{\theta})$ by a Gaussian distribution, for instance via a Laplace approximation. Using the entropy identity,
\[
\log \mathcal{L}_\mathrm{marginal}(\hat{\theta}; x) = \mathbb{E}_{b \sim p(\cdot \mid x, \hat{\theta})}\left[ \log \mathcal{L}(\hat{\theta}; x, b) \right] - \mathbb{E}_{b \sim p(\cdot \mid x, \hat{\theta})}\left[ \log p(b \mid x, \hat{\theta}) \right],
\]
we can further leverage independence across individuals, so that the entropy term decomposes as
\[
\mathbb{E}_{b \sim p(\cdot \mid x, \hat{\theta})}\left[ \log p(b \mid x, \hat{\theta}) \right] = \sum_{i=1}^n \mathbb{E}_{b_i \sim p(\cdot \mid x_i, \hat{\theta})}\left[ \log p(b_i \mid x_i, \hat{\theta}) \right].
\]

Approximating each $p(b_i \mid x, \hat{\theta})$ by a Gaussian distribution with covariance matrix $\Sigma_i$, we obtain
\[
\log \mathcal{L}_\mathrm{marginal}(\hat{\theta}; x) \approx \mathbb{E}_{b \mid x, \hat{\theta}}\left[ \log \mathcal{L}(\hat{\theta}; x, b) \right] + \sum_{i=1}^n \underbrace{\frac{1}{2} \left( \log \det \Sigma_i + q\left( \log(2\pi) + 1 \right) \right)}_{H_i},
\]
where $q$ denotes the dimension of the latent vector.

This approximation replaces the intractable posterior entropy with a sum of closed-form entropies of $q$-dimensional Gaussian distributions. Furthermore, each term $H_i$ depends only on its corresponding covariance matrix $\Sigma_i$, which can be easily estimated from MCMC samples of the latent variables using the empirical covariance estimator.
...
\[
    \hat{\chi}_i^{(1)} \coloneqq \mathbb{E}\left[ \chi^*(\mathcal{T}'_i) \mid \mathcal{Y}_i(t), \mathcal{S}_i(t), \hat{\theta} \right] \approx \frac{1}{B} \sum_{k=1}^B \chi^*(\mathcal{T}_i^{(k)}), \, \mathcal{T}_i^{(k)} \overset{\mathrm{i.i.d.}}{\sim} \mathscr{L}(\mathcal{T}_i^* \mid \mathcal{Y}(t), \mathcal{S}_i(t), \hat{\theta}),
\]
where the samples $\{ \mathcal{T}_i^{(k)} \}_{1 \leq k \leq B}$ are generated using a MCMC algorithm targeting the distribution $\mathscr{L}(b_i \mid \mathcal{Y}_i(t), \mathcal{S}_i(t), \hat{\theta})$ in conjunction with Algorithm~\ref{alg:trajectory-sim}.
...
\]
and approximated by a double Monte Carlo scheme accordingly.
...
\begin{example}[State at time $u$] \label{ex:state-at-time-u}
    Let $G = (V, E)$ be a finite directed acyclic graph, $u \in \mathbb{R}$ be a fixed time, and $\mathbb{Y} = \Delta(V)$ be the simplex of distributions over states. Let $\tau_i = \inf \{ n \in \mathbb{N}: T_{in} \geq u \text{ or } S_{in} \text{ is absorbing} \}$. Clearly, $\tau_i \leq \operatorname{depth}(G) < +\infty$. For each state $k \in V$, let $\mathds{1}_k \in \mathbb{Y}$ denote the vector that is $1$ at entry $k$ and $0$ elsewhere (a one-hot encoding of $k$). Then, we set $\chi_u^*(\mathcal{T}_i^*) = \mathds{1}_{S_{i\tau_i}} \in \mathbb{Y}$ to represent the state occupied by individual $i$ at time $u$. In particular, if $V = \{ 0, 1 \}$ and $E = \{ (0, 1) \}$, we recover the special case used for survival probability estimation in standard joint models \citep{rizopoulos2011dynamic}.
\end{example}
...
\]
and is applied consistently across all transitions. Covariates $X_i$ are normally distributed and enter linearly through $\beta^{k' \mid k} X_i$. We further assume $Q$ is diagonal, thus the $\log$-Cholesky parametrization reduces to the vector of negative $log$-standard deviations.
...
For each run, model parameters were estimated using the stochastic gradient ascent procedure described in Section~\ref{sec:inference}. Although the choice of initial parameters can significantly impact the optimization process, we set all initial parameter values to zero and initialize both covariance matrices $Q$ and $R$ as identity matrices, thereby avoiding any assumptions about prior knowledge of the parameters.

We used the Adam optimizer alongside an adaptive stopping criterion that monitors the first and second moments of parameter changes. Convergence was typically reached within $400$–$500$ iterations, requiring approximately $12$ seconds per run on an AMD Ryzen~5 processor. Formally, for any $t \geq 1$ and $i \in \{ 1, 2 \}$, consider $(\beta_1, \beta_2) = (0.99, 0.99)$ and
\[
    m_i^{(t)} \gets \beta_i m_i^{(t-1)} + (1 - \beta_i) (\theta^{(t)} - \theta^{(t-1)})^i, \quad \hat{m}_i^{(t)} = \frac{m_i^{(t)}}{1 - \beta_i^t},
\]
where all operations are understood element-wise, defining the first and second moment estimates of the successive increments of the parameters.

The stochastic optimization process is terminated when
\begin{equation} \label{eq:stop}
    \vert \hat{m}_1^{(t)} \vert \leq 10^{-6} + 10^{-1} \sqrt{\hat{m}_2^{(t)}},
\end{equation}
where the inequalities are required to hold component-wise.

\bigbreak

To illustrate the convergence of the estimator, we examine the optimization process for a particular run with $n = 1000$ in Figure~\ref{fig:fitting-plots}, as well as RMSE values computed from $n_\mathrm{runs} = 100$ independent runs on different simulated datasets, all generated from the same underlying distribution, as shown in Table~\ref{tbl:convergence-rmse}.

\begin{figure}[H]
    \centering
    \begin{tabular}{lcccc}
        \hline
        Parameter &True value &Mean &Std. deviation &RMSE \\
        \hline
        $\gamma_1$ &2.5000 &2.4960 &0.0370 &0.0372 \\
        $\gamma_2$ &-1.3000 &-1.3039 &0.0257 &0.0260 \\
        $\gamma_3$ &0.2000 &0.1936 &0.0267 &0.0275 \\
        $-\frac{1}{2} \log(Q_{11})$ &0.2554 &0.2243 &0.0442 &0.0540 \\
        $-\frac{1}{2} \log(Q_{22})$ &0.8047 &0.8009 &0.0260 &0.0263 \\
        $-\frac{1}{2} \log(Q_{33})$ &0.6020 &0.6001 &0.0276 &0.0277 \\
        $-\frac{1}{2} \log(R)$ &-0.2653 &-0.2684 &0.0083 &0.0089 \\
...
In particular, global cognitive functioning was assessed through the Mini-Mental State Examination (MMSE, see Figure~\ref{fig:paquid}), while physical dependency was evaluated using the HIER scale, which classifies subjects into four ordered states (see Figure~\ref{fig:hier-graph}): $0$ (no dependency), $1$ (mild dependency), $2$ (moderate dependency), and $3$ (severe dependency). Individual covariates correspond to a pair of binary variables indicating whether or not the individual has a diploma and is a male $\left( \mathds{1}_\mathrm{CEP}(i), \mathds{1}_\mathrm{male}(i) \right) \in \{0, 1\}^2$.
...
The transition graph was chosen as follows in Figure~\ref{fig:hier-graph}, and only allows monotonic transitions from a lower level of physical dependency to a higher one. We considered an exponential baseline hazard model for transitions, with a \emph{clock reset} specification, and the parameters of these exponential baseline hazards were jointly optimized with the model parameters.
...
Moreover, we imposed a specific substructure on the model by setting $\forall (k, k') \in E, \, \beta^{k' \mid k} = \beta$, which helped mitigate issues arising from the small number of observations in some groups. 

Additionally, since no prior information was available for the covariance matrix of the latent effects $Q \in \mathbb{R}^{3 \times 3}$, we used a full $\log$-Cholesky parametrization of the precision matrix $Q^{-1}$, with six parameters denoted $\tilde{Q}_1, \dots, \tilde{Q}_6$.

\subsection{Results}

In this study, we randomly split the data into a training set containing $80\%$ of the observations and a test set with the remaining $20\%$, using the training set to fit the model and the test set to evaluate its predictive performance.

\subsubsection{Inference}

The model was fitted using the Adam optimizer and the optimization process was run until convergence based on the Equation~\ref{eq:stop} as in the simulation study.

The Fisher Information Matrix was computed, implementing the method described in \cite{baey2023efficient}, directly taking into account shared parameters. The standard errors were then computed as the square root of the diagonal of the inverse of the Fisher Information Matrix. The results are summarized in Table~\ref{tbl:paquid-fit}.
...
\begin{figure}[H]
    ...
    \begin{tabular}{lcc|lcc}
        ...
        $\tilde{Q}_6$ &0.5220 &0.0470 &$-\frac{1}{2} \log(R)$ &\textbf{2.0830} &\textbf{0.0260} \\
        ...
        \hline
    \end{tabular}
    \caption{Inferred parameters and their estimated standard deviations. Bold values mean $p < 0.05$ (covariance parameters excluded).}
...
Using the $n_\mathrm{test} = 100$ individuals not included in the model fitting, we performed dynamic prediction through a single Monte Carlo integration scheme as defined in Section~\ref{sec:prediction}. For each individual, longitudinal measurements and trajectories were truncated at various times $t_\mathrm{trunc}$, and predictions were made for future time points $u \geq t_\mathrm{trunc}$ beyond the truncation. For any new individual $i$ in the test set, we consider the quantity $\chi_{u \wedge C_i}^*(\mathcal{T}_i^*)$ as defined by Example~\ref{ex:state-at-time-u}. It is estimated using the information available up to the conditioning (or truncation) time $t_\mathrm{trunc}$, \textit{i.e.}, $\left( \mathcal{Y}_i(t_\mathrm{trunc}), \mathcal{S}_i(t_\mathrm{trunc}) \right)$, according to the methodology of Section~\ref{sec:prediction}. Given an estimate $\hat{\chi}_{i, u \wedge C_i}$, the predicted state of the individual $i$ at time $u \wedge C_i$ is then defined as the most probable state
\[
s_i(t_\mathrm{trunc}) \in \operatorname*{\arg \max}_{s \in \{ 0, 1, 2, 3 \}} \left( \hat{\chi}_{i, u \wedge C_i} \right)_s
\]

For each fixed truncation time $t_\mathrm{trunc}$, common to all test individuals, accuracy was then assessed by comparing the predicted states with the true states at each corresponding future time point $u$, as illustrated in Figure~\ref{fig:paquid-acc}. Formally, the accuracy at time $u$ for predicted states $\left( s_i(t_\mathrm{trunc}) \right)_i$ is defined as
\[
    \mathrm{accuracy}(u) = \frac{1}{n_\mathrm{test}} \sum_{i=1}^{n_\mathrm{test}} \mathds{1}_{\chi_{u \wedge C_i}^*(\mathcal{T}_i^*) = \mathds{1}_{s_i(t_\mathrm{trunc})}}, 
\]
where each term in the sum equals one if and only if the predicted state matches the true state for individual $i$ at time $u \wedge C_i$.

Additionally, although $\mathcal{T}_i^*$ is unknown, the quantity $\chi_{u \wedge C_i}^*(\mathcal{T}_i^*)$ can be computed from the censored trajectory $\mathcal{T}_i$, since it does not depend on any events occurring after $C_i$. However, incorporating censoring times into prediction functions relies on information that is, in principle, not available at prediction time. Nevertheless, this inclusion is necessary to ensure fairness in accuracy metrics, since individuals who are not censored are generally more likely to have progressed to more advanced states of physical dependency than those who are censored.
...
\hfill
\begin{minipage}{0.45\textwidth}
    Here, each initial drop corresponds to a truncation time, before which accuracy remains at $100\%$ by design, since the predictor is almost surely constant given the observed history.
...
The simulation of the transition process $\mathcal{T}_i^*$ conditionally on $b_i$, $X_i$, and $(T_{i0}, S_{i0})$ can be achieved drawing on the semi-Markov property  by considering one transition at a time, according to Algorithm~\ref{alg:trajectory-sim}. The algorithm is similar to Gillespie's algorithm \citep{gillespie2007stochastic}, and can also include a survival condition $T_{i1} \geq t_{i, \mathrm{trunc}}$ that uses the Chasles relation.
...
\begin{algorithm}[H]
    ...
    \begin{algorithmic}[1]
        ...
        \Require subject-specific censoring time $C_i \in \mathbb{R} \cup \{ +\infty \}$; covariates $X_i$; latent variables $b_i$; initial time-state pair $(T_{i0}, S_{i0})$; optional survival condition $T_{i1} \geq t_{i, \mathrm{trunc}}$, defaults to $t_{i, \mathrm{trunc}} = -\infty$.
        \Statex
        ...
        \State Draw $T_{i\ell_i}^s$ such that $\forall t \geq T_{i(\ell_i-1)} \vee u$
        \Statex \hspace{\algorithmicindent} \hspace{\algorithmicindent} $-\log \mathbb{P}(T_{i\ell_i}^s > t) = \int_{T_{i(\ell_i-1)} \vee t_{i, \mathrm{trunc}}}^t \lambda_i^{s \mid S_{i(\ell_i-1)}}(w \mid b_i, X_i, T_{i(\ell_i-1)}, \theta)\, dw,$
        \EndFor
        \Statex
        \State Set $T_{i\ell_i} \gets \min_s T_{i\ell_i}^s$, and $S_{i\ell_i} \gets \arg \min_s T_{i\ell_i}^s$.
...
\begin{proof}
    First consider the case $t_{i, \mathrm{trunc}} = -\infty$. Let $t > T_{i\ell_i}$. One has
    \begin{align*}
        \mathbb{P}(T_{i(\ell_i+1)} > t \mid T_{i\ell_i}, S_{i\ell_i}) &= \mathbb{P}\left( \bigcap_{s : \, (E_{i(\ell_i-1)}, s) \in E} T_{i(\ell_i+1)}^s > t \mid T_{i\ell_i}, S_{i\ell_i} \right), \\
    ...
    If $t_{i, \mathrm{trunc}} > -\infty$, one can also check that both densities are equal. Note that the condtioning only affects the first transition, as the next time $\forall \ell_i > 0, \, T_{i\ell_i} \geq t_{i, \mathrm{trunc}} \implies \forall \ell_i > 0, \, T_{i\ell_i} \vee t_{i, \mathrm{trunc}} = T_{i\ell_i}$.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{jmstate-model-main/figures/stochastic-optimization.pdf}
    \caption{Evolution of the parameters during the optimization of the marginal $\log$-likelihood using stochastic gradient ascent. The dotted lines correspond to the true values.}
